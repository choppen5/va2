...
create table errorexceptions (
    ...
    time_exemption  timestamp,
    err_type_exept  char(10),
    note_rule_id    integer,
primary key (errorexceptions_id),
foreign key (note_rule_id) references notification_rule
        on delete set null
        on update cascade);
...
foreign key (com_server_id) references comunicationserver
        on delete cascade
        on update cascade,
foreign key (administrators_id) references administrators
        on delete restrict
        on update cascade);
...
foreign key (sft_mng_sys_id) references sft_mng_sys
        on delete set null
        on update cascade);
...
foreign key (sft_elmnt_id) references sft_elmnt
        on delete set null
        on update cascade);
...
foreign key (components_id) references components
        on delete cascade
        on update cascade,
foreign key (error_defs_id) references sft_error_defs
        on delete cascade
        on update cascade);
...
foreign key (analysis_rule_id) references analysis_rule
        on delete cascade
        on update cascade,
foreign key (error_defs_id) references sft_error_defs
        on delete cascade
