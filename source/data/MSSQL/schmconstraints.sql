...
*/
if exists (select * from sysdatabases where name='constraints')
...
*/
if db_name() = 'constraints'
    raiserror('''constraints'' database created, and context now in use.',1,1)
else
    raiserror('error in batch file, ''use constraints'' failed!  killing the spid now.',22,127) with log
 
go
 
execute sp_dboption 'constraints' ,'trunc. log on chkpt.' ,'true'
...
create index idx_server_task1
     on server_task (sv_name, tk_taskid, sft_elmnt_id)
