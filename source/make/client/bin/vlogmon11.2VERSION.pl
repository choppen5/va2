...
#	$obj = new Win32::AdvNotify || die "Can not create an AdvNotify object\n";
	...
	my $comp;
			...
			$numlines =
			  &getloglines(
				"$srvmgrvars{serverpath}\\log\\$srvmgrvars{enterprise}.$srvmgrvars{server_name}.log");
			...
			&parsefrmfstline("$srvmgrvars{serverpath}\\log\\$srvmgrvars{enterprise}.$srvmgrvars{server_name}.log");

			open (FH,"$srvmgrvars{serverpath}\\log\\$srvmgrvars{enterprise}.$srvmgrvars{server_name}.log") or $lh->log_print("dead $!");#
			...
			$lh->log_print("$srvmgrvars{serverpath}\\log\\");
			$lh->log_print("thread1 startING");
			#	$thread1 = $obj->StartThread(
			#		Directory    => "$srvmgrvars{serverpath}\\log\\",
			...
			eval {			
			$dwError = $obj->WatchDirectory("$srvmgrvars{serverpath}\\log\\", # szDirToWatch
                         	    8 | 2 | 1 | 3, # dwChangesToWatchFor
                         	    1 ,         # bWatchSubDirs     
								"$srvmgrvars{serverpath}\\log\\*", #filter
								""
                         	    );
			};
			
			if ($dwError) {$lh->fail("The directory couldn't be watched! ErrorCode: $dwError $srvmgrvars{serverpath}\\log\\ $@")}
	...
	if ($debug) {$lh->log_print("SERVER LINE = $line")};
		  ...
		  if ($tidsearch) {#only insert creation pids on startup
				...
				if  ($line !~ /Server Scheduler/ && $line =~ /ProcessCreate/) {
					...
					$line =~ /OS pid = (\d+)\)\s+for\s+([a-zA-Z_0-9-\s]+\(\w+\) |[a-zA-Z_0-9-\s]+)with task id (\d+)/; 
					
					#print "pid = $1\ncomp = $2\ntid = $3\n";
					#$pid{$3}{$2} = $1;
					if ($debug) {$lh->log_print("pid = $1 comp = $2 tid = $3")}

					vadmin::data1::insertpid($datasession,$debug,$lh,$srvmgrvars{server_name},$3,$1,undef,$2,$host,$appserver_id);
		...
		if ($tidsearch) {#only insert creation pids on startup
			...
			if ($line =~ /Process completed Successfully/) {
				...
				if ($debug) {$lh->log_print("PROCESS COMPLETE = $1 - Deleteing Key $1")};
			...
			if ($line =~ /Process exited with error/) {
				...
				if ($debug) {$lh->log_print("PROCESS EXITED WITH ERROR = $1 - Deleteing Key TID $1")};
				vadmin::data1::deletepidbytid($datasession,$debug,$lh,$1,$appserver_id);
			}	
			...
			if ($line =~ /Process was terminated/) {
				...
				#print "PROCESS EXITED WITH ERROR = $1 - Deleteing Key $1\n";
				if ($debug) {$lh->log_print("Process was terminate = $1 - Deleteing Key TID $1")};
				vadmin::data1::deletepidbytid($datasession,$debug,$lh,$1,$appserver_id);
			}	
	}
...
opendir (DIR, "$srvmgrvars{serverpath}\\log\\") || $lh->log_print("in findbasedontid could not open $srvmgrvars{serverpath}\\log\\");
	foreach $filename ( readdir DIR ) {
		my $file = $filename;
			if ($file =~ /(.*?)_$tid/ ) {
				...
				open( PFH, "$srvmgrvars{serverpath}\\log\\$file" )or $lh->log_print("tid: couldnt open $file");
				while (<PFH>) {
						if ($count < 1) {
...
sub createfilehash {
	...
	$size = (-s "$srvmgrvars{serverpath}\\log\\$srvrfile");
	$files{$srvrfile} = $size;


    opendir DIR1,
      "$srvmgrvars{serverpath}\\log"
      || $lh->log_print(
        "could not open $srvmgrvars{serverpath}\\log directory");
    foreach $filename ( readdir DIR1 ) {
        ...
        foreach $comp (@comps) {
            if ( $file =~ /$comp/) {
				...
				$size = (-s "$srvmgrvars{serverpath}\\log\\$filename");
				$files{$file} = $size;
                 # &getloglines("$srvmgrvars{serverpath}\\log\\$filename");
