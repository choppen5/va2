...
sub srvrcmd {
	...
	#$cmd,$harvestime,$growthwait,undef,$errorun
...
if ($refe->{STATUS} > 0) {
	...
	if ($cmd=~/list tasks for server/) {
		...
		%qhoa = $refe->parse_results;	
			...
			if ($runonce < 1) {
				...
				$runonce++;	
				#CO - modified 11/10/2007 - added $errorrun 
				srvrcmd($cmd,$harvestime,$growthwait,$runonce,$errorun);
			}   #recursive funciton!
 ...
 if ($qhoa{ERROR} < 30 && $qhoa{ERROR} > 0) {
	   ...
	   if ($errorun < 1) {
			$errorun++;	
			#CO - modified 11/10/2007 - added $runonce
			srvrcmd($cmd,$harvestime,$growthwait,$runonce,$errorun);
	   }
	   return 0;
   }
...
sub GetProcessInfo()
{
    ...
    if( $hProcess)
    {
		...
		if ($debug) {$lh->log_print("Could not open $Pid.. trying to force open" . Win32::FormatMessage (Win32::GetLastError ())) }
		%ProcInfo = ForceOpen($Pid);
...
{
      my $iResult = 0;
	 my $LookupPrivilegeValue = new Win32::API( 'AdvApi32.dll', 'LookupPrivilegeValue', ['P','P','P'], 'I' ) || $lh->log_print("Could not LookupPrivilegeValue"); 
	 my $AdjustTokenPrivileges = new Win32::API( 'AdvApi32.dll', 'AdjustTokenPrivileges', ['N','I','P','N','P','P'], 'I' ) || $lh->log_print("Could not AdjustTokenPrivileges");
	...
	$LookupPrivilegeValue= undef;
	$AdjustTokenPrivileges= undef;
	return( $iResult );
}