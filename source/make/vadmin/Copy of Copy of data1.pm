...
my $incvar = &erroridincr;
	...
	if ($db->Sql($SqlStatement)) {
	   $lh->log_print("SQL insert error event failed - $SqlStatement.\n");
	  my ($ErrNum, $ErrText, $ErrConn) = $db->Error();
      $lh->log_print("ERRORS: $ErrNum $ErrText $ErrConn");
	  processdberror($ErrNum,$lh);
	   $rethash =   "Error: " . $db->Error();
	   return  $rethash;
	   # add some function to try to reinitialize db session
	}  
	...
	sub erroridincr {
	my $rethash;
	$SqlStatement = "select max(errorevent_id)  \"increment\" from errorevent";
	if ($db->Sql($SqlStatement)) {
...
sub processdberror {
	...
	if ($error == 911) {
		$lh->log_print("COMMUNICATION LINK FAILURE! - Sleeping for 120 seconds");
		sleep(120);
	}
