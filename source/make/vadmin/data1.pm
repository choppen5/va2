	...
	if (&execsql($db,$debug,$lh,$SqlStatement)) {
		  ...
		  while($db->FetchRow()) {
			%Data = $db->DataHash();
			 ...
			 $initvars{logdir} = $Data{"logdir"};
		  }

		my $hashkey;
		foreach  $hashkey (keys %Data) {
			print "RETURNED DATA HASH = $hashkey = $Data{$hashkey}\n"; 
		}
...
####################################################
# CO 11/9/2007 - updated to compatible with siebel 8 by counting the columns returened
my $columncount = @fl;
my $siebversion;
my $sqlinsert;
my $sqlupdate;
...
# CO 4/11/2009 - added for Canal+ - french compnent status has ' in it, this will pad all ' 
foreach my $item (@fl) {
	$item =~ s/\'/\'\'/g;
}

$incvar = &keyincr($db,$debug,$lh,"monitored_comps","monitored_comps_id");
if ($columncount > 15) {
	$siebversion = 8;
	$sqlinsert = "insert into monitored_comps (monitored_comps_id,sft_elmnt_id,sv_name,cc_alias,cc_name,ct_alias,cg_name,cc_runmode,cp_disp_run_state, cp_startmode, cp_num_run,cp_max_tas,cp_actv_mt,cp_max_mts,cp_start_time,cp_end_time,cp_status) values ($incvar,$server_id,\'$fl[0]\',\'$fl[1]\' , \'$fl[2]\', \'$fl[3]\', \'$fl[4]\',\'$fl[5]\', \'$fl[6]\',\'$fl[7]\',\'$fl[8]\',\'$fl[9]\',\'$fl[10]\', \'$fl[11]\', \'$fl[12]\',\'$fl[13]\',\'$fl[14]\')";
	$sqlupdate = "update monitored_comps set cc_alias = \'$fl[1]\' ,cc_name = \'$fl[2]\',ct_alias = \'$fl[3]\',cg_name= \'$fl[4]\',cc_runmode = \'$fl[5]\',cp_disp_run_state = \'$fl[6]\',cp_num_run = \'$fl[8]\',cp_max_tas = \'$fl[9]\',cp_actv_mt = \'$fl[10]\',cp_max_mts = \'$fl[11]\',cp_start_time = \'$fl[12]\',cp_end_time = \'$fl[13]\',cp_status = \'$fl[14]\', cp_startmode = ''$fl[8]\' where cc_alias = \'$fl[1]\' and sft_elmnt_id = $server_id" ;
} else {
	...
	$sqlupdate = "update monitored_comps set cc_alias = \'$fl[1]\' ,cc_name = \'$fl[2]\',ct_alias = \'$fl[3]\',cg_name= \'$fl[4]\',cc_runmode = \'$fl[5]\',cp_disp_run_state = \'$fl[6]\',cp_num_run = \'$fl[7]\',cp_max_tas = \'$fl[8]\',cp_actv_mt = \'$fl[9]\',cp_max_mts = \'$fl[10]\',cp_start_time = \'$fl[11]\',cp_end_time = \'$fl[12]\',cp_status = \'$fl[13]\' where cc_alias = \'$fl[1]\' and sft_elmnt_id = $server_id" ;
}
...
$tk_pid = &turntozero($tk_pid);
	...
	if ($tasksync =~ /INSERT/) {
		...
		#CO 11/10/2007 - changed so the task state has to be running too before inserting from tasks.
		if ($tk_pid && $tk_disp_runstate =~ /Running/) {
...
sub processdberror {
	...
	if ($error == 911 || $error == 10054 || $error == 230 || $error == 3114 || $error == 2006) {
