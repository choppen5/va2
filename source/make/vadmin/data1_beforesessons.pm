...
if ($tasksync) {		
		...
		if ($debug) {$lh->log_print("server task insert statement = $SqlStatement")}
		&execsql($db,$debug,$lh,$SqlStatement);	
		insertpid($db,$debug,$lh,$sv_name,$tk_taskid,$tk_pid,$cc_alias,undef,$host,$server_id);
			...
			my %insertcheck = &gethashrecord($db,$debug,$lh,"select tk_taskid \"tk_taskid\" from server_task where tk_taskid = $tk_taskid and sft_elmnt_id  = $server_id");

			unless ($insertcheck{tk_taskid}) {

				#$fl[0]\',\'$fl[1]\' , \'$fl[2]\', \'$fl[3]\', \'$fl[4]\',\'$fl[5]\', \'$fl[6]\',\'$fl[7]\',\'$fl[8]\',\'$fl[9]\','$fl[10]','$fl[11]','$fl[12]','$fl[13]')";
