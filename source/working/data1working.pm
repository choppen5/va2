...
$threads = &turntozero($threads);
	...
	if ($insertcheck{pid}) {
		...
		$SqlStatement = "insert into processes (process_id,pid,host,sft_elmnt_id,process,cpu,cpu_time,kernel_time,user_time,memory,pagefaults,virtualmem,priority,threads) values ($incvar,$pid,'$host',$sft_elmnt_id,$process,$cpu,$cpu_time,$kernel_time,$user_time,)";
	}
