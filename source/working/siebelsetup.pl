	...
	if ($count == 2) {
		...
		unless ($version) {
			...
			if ($version =~ /7/) {
				...
				#
				$lh->log_print("Could not identify version number.  Please contact support at www.recursivetechnology.com to verify version."); 
				print("Please contact support at www.recursivetechnology.com to verify version.\n");
				print("Enter version number to continue:\n");
					$versionstring = <STDIN>;
					chomp($version);
...
########################################
$lh->log_print("Identified Siebel Version: $version");
...
########  Insert enterprise	
	...
	#get dns name
	#[/enterprises/SBA_80/named subsystems/ServerDataSrc/parameters/DSConnectString]
	...
	if ($version  == 6) {
	   $dns = &returnvalue($bigstring);
		...
		$bigstring = "\/enterprises/".$_."\/named subsystems\/ServerDataSrc\/parameters\/DSConnectString";
		print "retrieving siebel 8 datasourc $bigstring\n";
		$dns = &returnvalue($bigstring);
	} else 	{
	...
	my @appservers = &appsforent($_);
		...
		foreach  (@appservers) {
			...
			my $apphost;
			if ($version  == 8) {
				$apphost = &returnvalue7($bigstring);
			} else {
				$apphost = &returnvalue($bigstring);
			}
			
			chomp($apphost);
			print "apphost = $apphost\n";
			...
			$lh->log_print("Inserted appserver $_,$apphost,$installdir,$service_name,$entid");
...
sub gatewayinsert {
	...
	my $id; #returned from db or inserted
	
	#check for gateway... 1. select gateway for this p
	
	my $alreadyexists;
	my $SqlStatement = "select  sft_elmnt_id \"sft_elmnt_id\" from sft_elmnt where sft_mng_sys_id = $par_prod_id and type = 'gateway' and host = '$host'"	;

		if (my $db = &execsql($datasession,$debug,$lh,$SqlStatement)) {
	
		  while($db->FetchRow()) {
			    my %Data = $db->DataHash();
				$id= $Data{"sft_elmnt_id"};
			}
		}
	...
	if ($id) {
		...
		#insert a new gateway one

		$id = keyincr($datasession,$debug,$lh,'sft_elmnt','sft_elmnt_id');
...
sub returnvalue8 {
	#RETURNS the value that is the third parameter instead of second
	my $bigstring = shift; #shift function takes the first value passed to a sub
	my $count = 0;
	my ($key,$value);
	foreach  (@contents) {  #loop through the whole file, again
		...
		if ($_=~$bigstring) {		#if we find a match
			...
			($key,$value) = split(/=/,$contents[$count+2]); #split function splists arrays - we've mapped the fact 
															#that the value line will always be +3 
			$value =~ s/\"//g;								#remove quotes... i think there is a easier way	
			return $value;
